...
class Model_users extends CI_Model
{
   ...
   public function getDataListJabatan()
   {
	    $this->db->where('id_status', '1');
	    $this->db->order_by('id_jabatan ASC');
		$query = $this->db->get('xi_sa_jabatan');

	    $dd_jab[''] = 'Pilih Data';
	    if ($query->num_rows() > 0) {
	      foreach ($query->result() as $row) {
	        $dd_jab[$row->id_jabatan] = $row->nm_jabatan;
	      }
	    }
	    return $dd_jab;
   }

   public function getDataJabatan($opdId)
   {
	    $this->db->where('id_status', '1');
	    $this->db->where('id_opd', $opdId);
	    $this->db->order_by('id_jabatan ASC');
		$query = $this->db->get('xi_sa_jabatan');
   ...
   public function getDetailJenisUser($id_jabatan)
   {
	    $this->db->where('id_status', '1');
	    $this->db->where('id_jabatan', $id_jabatan);
		$query = $this->db->get('xi_sa_jabatan');

	    return $query->row_array()['jenis_user'];
   }
  ...
  private function _get_datatables_query($param)
  {
	...
	public function insertDataUsers()
	{
		...
		$id_jabatan    = escape($this->input->post('id_jabatan', TRUE));
		...
		$data = array(
			...
			'jenis_user' 			=> escape($this->getDetailJenisUser($id_jabatan)),
			'validate_email_code'	=> '',
  ...
  public function updateDataUsers()
  {
    ...
    public function countAgenda($users)
    {
    	$this->db->where_in('penerima', $users);
