...
class Home extends SLP_Controller {
	...
	public function createForm()
	{
		...
		$this->session_info['page_name'] = "Ganti Sandi";

   		$this->template->build('vsandi', $this->session_info);
	}

	private function createData()
	{
		if($this->mhome->validasiDataSandi() == FALSE) {
			error_message('danger', 'Peringatan!', 'Tolong dilengkapi form inputan dibawah...');
      $this->createForm();
    } else {
			$data = $this->mhome->updatePassword();
			if($data) {
				error_message('success', 'Sukses!', 'Sandi Berhasil Diubah.');
				redirect('home/sandi');
			}
		}
	}
