        ...
        <div class="col-xs-12 col-sm-12">
          ...
          <div class="pull-right">
            <div class="btn-toolbar">
              <!-- <button type="button" class="btn btn-primary btn-sm" id="printDoc"><i class="fa fa-file-word-o"></i> EXPORT KE WORD</button>
              <button type="button" class="btn btn-success btn-sm" id="printExcel"><i class="fa fa-file-excel-o"></i> EXPORT KE EXCEL </button>
              <button type="button" class="btn btn-danger btn-sm" id="printPdf"><i class="fa fa-file-pdf-o"></i> EXPORT KE PDF </button> -->
            </div>
          </div>
        </div>
        <div class="col-xs-12 col-sm-12">
            ...
            <div style="display:block;background:#FFF;padding:20px;border:1px solid #CCC;box-shadow:0px 0px 10px #CCC;">
              ...
              <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    ...
                    <?php echo form_dropdown('status', array(''=>'Pilih Status', 'SM'=>'Menunggu', 'SH'=>'Dihadiri', 'SW'=>'Diwakilkan'), $this->input->post('status'), 'class="select-all" ');?>
                  </div>
                </div>
              </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
         ...
         <div class="col-xs-12 col-sm-4">
          ...
          <div class="col-xs-12 col-sm-4">
            <div class="form-group required">
              ...
              <input type="text" class="form-control timepicker1" name="jam_mulai" id="jam_mulai" placeholder="Jam Mulai" value="<?php echo $this->input->post('jam_mulai', TRUE); ?>">
          ...
          <div class="col-xs-12 col-sm-4">
            <div class="form-group required">
              ...
              <input type="text" class="form-control timepicker1" name="jam_selesai" id="jam_selesai" placeholder="Jam Selesai" value="<?php echo $this->input->post('jam_selesai', TRUE); ?>">
              <div class="help-block"></div>
            </div>
          </div>
        </div>
        <div class="row">
         ...
         <div class="col-xs-12 col-sm-6">
            <div class="form-group required">
              <label for="penyelenggara" class="control-label" style="font-size:15px;"><b>Penyelenggara Kegiatan <font color="red">*</font></b></label>
              <textarea class="form-control" name="penyelenggara" id="penyelenggara" placeholder="Penyelenggara Kegiatan"></textarea>
              <div class="help-block"></div>
            </div>
          </div>
          <div class="col-xs-12 col-sm-6">
            <div class="form-group required">
              <label for="cp" class="control-label"><b>Contact Person <font color="red">*</font></b></label>
               <textarea class="form-control" name="cp" id="cp" placeholder="Kontak Person"></textarea>
              <div class="help-block"></div>
            </div>
          </div>
        </div>
...
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/plugins/jqueryui/jquery.ui.timepicker.css" rel="stylesheet" /> 
<script type="text/javascript"src="<?php echo base_url();?>assets/plugins/jqueryui/jquery.ui.timepicker.min.js"></script>
<script type="text/javascript">
...
<script type="text/javascript">
  ...
  $(document).on('change', 'select[name="jenis_agenda"]', function(e) {
    let id = $(this).val();
    getPenerima(id);
  });

  //mengambil data kab/kota
  function getPenerima(jenis_agenda) { 
    regeID = (regeID != '') ? regeID : '<?php echo $this->input->post('penerima', TRUE); ?>'; 
    //agendaID = (agendaID != '') ? agendaID : '<?php echo $this->input->post('jenis_agenda', TRUE); ?>'; 
    let lblReg  ='';
    $.ajax({
      type: 'GET',
      ...
      success: function(data) {
        $('input[name="'+csrfName+'"]').val(data.csrfHash);
        $('select[name="penerima"]').html('').select2('data', null);
        if(data.status == 1) {
          lblReg = '<option value="">Pilih Penerima</option>';
          $.each(data.message,function(key,value){
            lblReg += '<option value="'+value['id']+'" selected>'+value['text']+'</option>';
          });
        } else{
          lblReg = '<option value="">Pilih Penerima</option>';
        }
        $('select[name="penerima"]').html(lblReg);
        $('select[name="penerima"]').select2('val', regeID).trigger('change');
       // $('select[name="jenis_agenda"]').select2('val', agendaID).trigger('change');
      }
    });
  }
</script>
