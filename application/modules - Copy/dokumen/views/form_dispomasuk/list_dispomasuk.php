              ...
              <div class="row">
               ...
               <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Tanggal Disposisi</b></label>
                ...
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Pilih Status Baca</b></label>
                    <?php echo form_dropdown('status', array(''=>'Pilih Status', 'BD'=>'Belum Dibaca', 'SD'=>'Sudah Dibaca'), $this->input->post('status'), 'class="select-all" id="status_id"');?>
                  </div>
                </div>
              </div>
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="panel panel-primary">
        <div class="panel-heading">
          <h4>Daftar Disposisi Masuk</h4>
        </div>
        <div class="panel-body collapse in">
          <div class="table-responsive">
            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="tblDispomasuk" width="100%">
              <thead>
                <tr>
                  <th width="3%">#</th>                  
                  <th width="10%"> Isi Disposisi</th>
                  <th width="20%"> Pengirim Disposisi</th>                  
                  <th width="10%"> Tanggal Disposisi</th>
                  <th width="20%"> Penerima Disposisi</th> 
...
<div class="modal fade in" id="modalEntryForm" tabindex="-1" role="dialog" aria-labelledby="modalEntryLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" id="frmEntry">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart(site_url('dokumen/dispomasuk/create'), array('id' => 'formEntry')); ?>
...
<div class="modal fade in" id="modalEntryFormReview" tabindex="-1" role="dialog" aria-labelledby="modalEntryLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" id="frmEntryReview">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart(site_url('dokumen/dispomasuk/approve'), array('id' => 'formEntryReview')); ?>
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
...
<script type="text/javascript">
  ...
  function getDataListAgenda() {
    $('#tblDispomasuk').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "dokumen/dispomasuk/listview",
        "type": "POST",
        "data": {
    ...
    $('#tblDispomasuk_filter input').addClass('form-control').attr('placeholder','Search Data');
    $('#tblDispomasuk_length select').addClass('form-control');
  }
  ...
  function formReset() {
    $('#formEntry').attr('action', site + 'dokumen/dispomasuk/create');
    $('#errEntry').html('');
    $('.select-all').select2('val', '');
  ...
  $(document).on('click', '.btnEdit', function(e){
    formReset();
    $('#formEntry').attr('action', site + 'dokumen/dispomasuk/update');
  ...
  function getDetailAgenda(token) {
    ...
    $.ajax({
      type: 'POST',
      url: site + 'dokumen/dispomasuk/details',
      ...
      buttons: {
        ...
        "main" : {
          ...
          callback:function(response){
            if (response) {
              $.ajax({
                url: site + 'dokumen/dispomasuk/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
  ...
  $(document).on('click', '.btnReview', function(e){
    formResetReview();
    $('#formEntryReview').attr('action', site + 'dokumen/dispomasuk/approve');
    var token = $(this).data('id');
  ...
  $(document).on('click', '.btnReject', function(e){
    formReset();
    $('#formEntryReview').attr('action', site + 'dokumen/dispomasuk/reject');
    var token = $(this).data('id');
    var nama_agenda = $(this).data('nm');
