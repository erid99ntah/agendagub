  ...
  <div class="row" id="formParent">
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <!-- <button type="button" id="getDprd" class="btn btn-primary-alt" name="button" style="padding:12px 16px;"><b><i class="fa fa-refresh"></i> Get Data DPRD</b></button>       
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">        
         ...
         <?php echo form_open(site_url('#'), array('id'=>'formSinkronisasi', 'style'=>'margin-bottom:20px;')); ?>
            <div style="display:block;background:#FFF;padding:20px;border:1px solid #CCC;box-shadow:0px 0px 10px #CCC;">
              <div class="row">                
                <div id="errEntry"></div>
               
                <div class="col-xs-12 col-sm-6">
                 <label for="username" style="font-size:16px;"><b>* Klik Tombol Get Data Aspirasi untuk sinkronisasi data dengan Aplikasi Aspirasi DPRD</b></label><br>
                    <!-- <button type="submit" class="btn btn-primary" name="saveKegiatan" id="btnInsertKegiatan"><i class="fa fa-refresh"></i> Get Data Aspirasi</button> -->
                    <a href="<?php echo site_url('manajemen/dprd/create');?>" class="btn btn-primary" ><i class="fa fa-refresh"></i> Get Data Aspirasi</a>
                </div>
...
<div class="modal fade in" id="modalEntryForm" tabindex="-1" role="dialog" aria-labelledby="modalEntryLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" id="frmEntry">
    <div class="modal-content">
      ...
      <?php echo form_open(site_url('manajemen/dprd/create'), array('id' => 'formEntry')); ?>
...
<script type="text/javascript">
  ...
  function getDataListUser() {
    $('#listuser').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "manajemen/dprd/listview",
        "type": "POST",
        "data": {
  ...
  function formReset() {
    $('#formEntry').attr('action', site + 'manajemen/dprd/create');
    $('#errEntry').html('');
  ...
  $(document).on('click', '.btnEdit', function(e){
    ...
    $('#formEntry').attr('action', site + 'manajemen/dprd/update');
    $('.lblPass').text('');
    $('#modalEntryForm').modal({
      backdrop: 'static'
    });
  ...
  function getDataUser(token) {
    ...
    $.ajax({
      type: 'POST',
      url: site + 'manajemen/dprd/details',
      ...
      success: function(dataUser) {
        ...
        if(dataUser.status == 1) {
          ...
          /*var dataSub = {
              id: dataUser.message.sub_opd,
              text: dataUser.message.nm_sub_opd
          };

          var newOption = new Option(dataSub.text, dataSub.id, false, false);
          $('#sub_opd').append(newOption).trigger('change');*/
      ...
      buttons: {
        ...
        "main" : {
          ...
          callback:function(response){
            if (response) {
              $.ajax({
                url: site + 'manajemen/dprd/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
  ...
  function checkUsername(newname) {
		$.ajax({
      type: 'POST',
      url: site + 'manajemen/dprd/searching',
  ...
  function getRegency(provinceId) {
    ...
    $.ajax({
      type: 'GET',
      url: site + 'manajemen/dprd/subOpd',
      data: {'opdId' : opdId},
      dataType: 'json',
      success: function(data) {
        $('input[name="'+csrfName+'"]').val(data.csrfHash);
  ...
  {
      $.ajax({
       url      :"<?php echo site_url('manajemen/dprd/subOpd'); ?>",
       method   :"GET",
       dataType :"html",
       data: {'opdId' : opdId},
       beforeSend:function(){          
  ...
  $(document).on('click', '#getDprd', function(e){
    getDataDprd();
    alert('asdasd');
  });

  function getDataDprd()
  {
      $.ajax({
       url      :"<?php echo site_url('manajemen/dprd/create'); ?>",
       method   :"GET",
       dataType :"html",
        data: {'<?php echo $this->security->get_csrf_token_name(); ?>' : $('input[name="'+csrfName+'"]').val()},
       beforeSend:function(){          
          $('#getDprd').attr('disabled', 'disabled');          
          run_waitMe($('#formParent'));
       },
       success:function(data)
       {
        console.log(data.status);
          if(data.status == 0) {
          $('#errSuccess').html('<div class="alert alert-danger" id="pesanErr"><strong>Data Gagal Di Sinkronisasi</div>');
                         
          } 
          else {
          $('#errSuccess').html('<div class="alert alert-dismissable alert-success">'+
                                    '<strong>Sukses!</strong> '+ data.status +
                                    '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>' +
                                  '</div>');
          }
          getDataListUser();   
       ...
       error:function(){          
         $('#errSuccess').html('<p class="error"><strong>Oops!</strong> Try that again in a few moments.</p>');
       }
      });
  }  

  $('#formSinkronisasi').submit(function(e){
      e.preventDefault();
      $.ajax({
       url    :"<?php echo site_url();?>manajemen/dprd/create",
       method :"POST",
       ...
       {  
          $('#errEntry').html('');
          $('#btnInsertKegiatan').attr('disabled', 'disabled');
          $('#process').css('display', 'block');
          run_waitMe($('#formSinkronisasi'));
          $('#errEntry').html('');
       },
       success:function(data)
       {
          $('#formSinkronisasi').waitMe('hide');
          $('#btnInsertKegiatan').attr('disabled', false);       
